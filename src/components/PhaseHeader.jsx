  ...
  return (
    <div className={`flex items-center justify-between border-b border-slate-800 transition-all ${isPresentation ? 'mb-6 pb-4' : 'mb-6 pb-4'}`}>
      <div className="flex items-center gap-4">
        <div className={`rounded-lg border border-red-500/30 flex items-center justify-center shadow-[0_0_15px_rgba(239,68,68,0.15)] ${isPresentation ? 'w-20 h-20 bg-red-900/30' : 'w-12 h-12 bg-red-900/20'}`}>
          <Icon size={isPresentation ? 40 : 24} className="text-red-400" />
        </div>
        <div>
          <h4 className={`font-bold text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-orange-400 uppercase tracking-widest ${isPresentation ? 'text-lg mb-1' : 'text-[10px] mb-0.5'}`}>
            {phase}
          </h4>
      ...
      {time && (
        <div className={`flex items-center gap-2 text-slate-400 font-mono bg-slate-900/80 rounded-full border border-slate-700 backdrop-blur-md ${isPresentation ? 'text-xl px-6 py-3' : 'text-xs px-3 py-1.5'}`}>
          <Clock size={isPresentation ? 24 : 14} className="text-red-500" /> {time}
        </div>
      )}
    </div>
  )
