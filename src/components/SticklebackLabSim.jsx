...
export default function SticklebackLabSim({ isPresentation }) {
  ...
  return (
    <div className="h-full flex flex-col font-mono text-xs md:text-sm bg-stone-900 border-4 border-stone-800 rounded-xl relative overflow-hidden shadow-2xl">
      {/* Header */}
      <div className="flex justify-between items-center p-4 border-b-2 border-stone-800 bg-stone-950 z-10">
        <div className="flex items-center gap-2">
          <Fish size={16} className="text-emerald-500" />
          <span className="text-emerald-500 font-bold tracking-widest font-serif">TINBERGEN_LAB_1951</span>
        </div>
        <div className="text-stone-500">SUBJECT: GASTEROSTEUS ACULEATUS</div>
      </div>

      <div className="flex-grow flex relative z-10">
        
        {/* Sidebar: Controls & Log */}
        <div className="w-1/3 border-r-2 border-stone-800 bg-stone-950 p-4 flex flex-col gap-4">
          {phase === 'intro' ? (
            <div className="h-full flex flex-col justify-center text-center">
              <h3 className="text-xl font-bold text-white mb-4 font-serif">HYPOTHESIS</h3>
              <p className="text-stone-400 mb-6">"Aggression in sticklebacks is an innate FAP triggered by a specific Sign Stimulus."</p>
              <button onClick={() => setPhase('experiment')} className="bg-emerald-700 text-white py-3 rounded font-bold hover:bg-emerald-600">BEGIN EXPERIMENT</button>
            </div>
          ) : phase === 'experiment' ? (
            <>
              <div>
                <span className="text-stone-500 block text-[10px] uppercase mb-2">Select Stimulus</span>
                <div className="grid grid-cols-1 gap-2">
                  {stimuli.map(s => (
                    <button 
                      ...
                      className={`p-3 border rounded text-left transition-all ${activeStimulus?.id === s.id ? 'bg-amber-900/50 border-amber-500' : 'bg-stone-900 border-stone-700 hover:bg-stone-800'}`}
                    >
              ...
              <div className="flex-grow flex flex-col min-h-0">
                <span className="text-stone-500 block text-[10px] uppercase mb-2 mt-4">Field Notes</span>
                <div className="flex-grow overflow-y-auto bg-stone-900/50 p-2 rounded border border-stone-800 custom-scrollbar">
              ...
              {completed && (
                <div className="mt-2">
                  <p className="text-xs text-stone-400 mb-2">Sufficient data collected.</p>
                  <div className="flex gap-2">
                    <button onClick={() => submitConclusion('shape')} className="flex-1 py-2 bg-stone-800 text-stone-300 border border-stone-600 hover:bg-stone-700 rounded text-[10px]">Trigger = SHAPE</button>
                    <button onClick={() => submitConclusion('red')} className="flex-1 py-2 bg-amber-900/50 text-amber-200 border border-amber-700 hover:bg-amber-800 rounded text-[10px]">Trigger = RED SPOT</button>
                  </div>
                </div>
              )}
            </>
            ...
            <div className="h-full flex flex-col justify-center text-center p-4">
              <div className={`text-4xl mb-4 ${conclusion.correct ? 'text-green-500' : 'text-red-500'}`}>
                {conclusion.correct ? <CheckCircle size={64} className="mx-auto"/> : <X size={64} className="mx-auto"/>}
              </div>
              <h3 className="text-xl font-bold text-white mb-2">{conclusion.correct ? "HYPOTHESIS CONFIRMED" : "HYPOTHESIS REJECTED"}</h3>
              <p className="text-stone-400 mb-6">{conclusion.msg}</p>
              <button onClick={() => { setPhase('intro'); setLog([]); setCompleted(false); setConclusion(null); }} className="text-stone-500 underline">Reset Lab</button>
            </div>
          )}
        </div>

        {/* The Tank Visual */}
        <div className="w-2/3 bg-cyan-900/20 relative overflow-hidden flex items-center justify-center border-l border-stone-800">
          {/* Water Effect */}
