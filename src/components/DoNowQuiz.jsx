  ...
  return (
    <div className={`grid grid-cols-1 lg:grid-cols-2 h-full content-start transition-all ${isPresentation ? 'gap-16' : 'gap-8'}`}>
      <div className="space-y-6">
        <div className={`bg-gradient-to-br from-cyan-900/20 to-slate-900 rounded-2xl border border-cyan-500/20 p-8 shadow-lg relative overflow-hidden`}>
          ...
          {!showResults ? (
            <>
              <button
                ...
                className={`bg-cyan-700 hover:bg-cyan-600 disabled:opacity-50 disabled:grayscale text-white font-bold rounded-xl w-full transition-all shadow-lg ${isPresentation ? 'px-12 py-8 text-3xl' : 'px-8 py-4'}`}
              >
                Analyze Results
              </button>
            </>
      ...
      <div className="space-y-3 overflow-y-auto pr-2 custom-scrollbar max-h-full">
        {questions.map((q) => (
          <div key={q.id} className={`bg-slate-900 border border-slate-800 rounded-xl hover:border-cyan-700 transition-colors ${isPresentation ? 'p-6 mb-4' : 'p-4'}`}>
            <h4 className={`font-bold text-cyan-100 mb-3 ${isPresentation ? 'text-3xl' : 'text-sm'}`}>
              <span className="text-cyan-600 mr-2 font-mono">Q{q.id}</span>
              {q.question}
            </h4>
            {isPresentation ? (
